...
class ArpEnumerator(object):
         def __init__(self, interfae=False, passive=False, range=False):
...
def passive_handler(self, pkt):
        try:
                if not pkt[ARP].psrc in self.discovered_hosts.keys():
                        ...
                        self.discovered_hosts[pkt[ARP].psrc] = pkt[ARP].hwrc
        except Exception:
                return
...
def active_scan(self):
        ...
        for snd, rcv, in ans:
                 self.discovered_hosts[recv[ARP].psrc] = rcv[ARP].hwrc
                 print '%s - %s' %(recv[ARP].psrcc, rcv[ARP].hwsrc)
        print '\n[*] Scan Compelete'
        self.duration = datetime.now() - self.starttime
